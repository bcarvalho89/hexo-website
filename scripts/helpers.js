...
hexo.extend.helper.register('header_menu', function(className) {
  ...
  var isPortuguese = lang === 'pt-br';

  _.each(menu, function(path, title) {
    if (!isPortuguese) path = lang + path;

    result += '<li><a href="' + self.url_for(path) + '" class="' + className + '-link">' + self.__('menu.' + title) + '</a></li>';
  });
...
hexo.extend.helper.register('canonical_url', function(lang) {
  ...
  if (lang && lang !== 'pt-br') path = lang + '/' + path;
...
hexo.extend.helper.register('url_for_lang', function(path) {
  ...
  if (lang !== 'pt-br' && url[0] === '/') url = '/' + lang + url;
